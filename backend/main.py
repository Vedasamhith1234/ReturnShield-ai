...
# ---------------------------------------------------------------------------
# Enterprise features: analyst comments, live transcript analysis, invoice
...
# In-memory comment store: {return_id: [comment, ...]}
# In production this is a database table with author auth.
_COMMENTS = {}
...
def get_comments(return_id: str):
    return _COMMENTS.get(return_id, [])
...
def add_comment(return_id: str, comment: CommentIn):
    ...
    _COMMENTS.setdefault(return_id, []).append(entry)
    return {"ok": True, "comments": _COMMENTS[return_id]}
...
class CustomerReturnIn(BaseModel):
    ...
    description: str = ""
    image_meta: dict = {}   # output of client-side/vision analysis if available


@app.post("/api/submit-return")
def submit_customer_return(body: CustomerReturnIn):
    """Customer-facing intake: runs the full pipeline on the submission and
    queues the case for the company dashboard. Returns ONLY a customer-safe
    status — never fraud scores or reasons (anti-fraud design: don't tip off
    bad actors about what triggered review)."""
    ...
    order = {
        ...
        "coupon_used": False,
        "warehouse": "DFW1",
        "order_date": (now - timedelta(days=body.days_since_delivery + 3)).isoformat(),
        "delivery_date": (now - timedelta(days=body.days_since_delivery)).isoformat(),
    ...
    return_row = {
        ...
        "days_before_return": body.days_since_delivery,
        "gps_mismatch_km": 0.0,
    ...
    chat_row = {
        ...
        "transcript": f"Customer (return form): {body.description}" if body.description else "(no description provided)",
        "used_copy_paste_excuse": False,
    }
    image_row = {
        "sku_match": body.image_meta.get("sku_match", True),
        "staged_damage_suspected": body.image_meta.get("staged_damage_suspected", False),
        "reused_photo_detected": body.image_meta.get("reused_photo_detected", False),
        "serial_number_match": body.image_meta.get("serial_number_match", True),
    }
    CHATS[return_row["return_id"]] = chat_row
    IMAGES[return_row["return_id"]] = image_row
    RETURN_FREQ[order["customer_id"]] = RETURN_FREQ.get(order["customer_id"], 0) + 1

    case = run_case(return_row, order, chat_row, image_row, RETURN_FREQ[order["customer_id"]])
