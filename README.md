...
**Camera note:** browsers require a "secure context" for camera access.
Opening the file directly (`file://`) may block the camera in some browsers —
if so, use "Upload photo instead", or serve the dashboard via the backend
below and open `http://localhost:8000` instead (localhost counts as secure).
