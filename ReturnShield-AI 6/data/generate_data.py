...
# Coherent, generic follow-up detail a customer might type in response to
# "Can you tell me more?" — used instead of fake.sentence(), which produces
# grammatically-random word salad (real words, no meaning) that reads as
# obviously fake to anyone viewing the transcript in the dashboard.
CUSTOMER_FOLLOWUP_DETAILS = [
    "It just stopped working properly out of nowhere.",
    "I noticed the issue as soon as I opened the box.",
    "It's been pretty inconvenient trying to use it like this.",
    "I already tried a few basic troubleshooting steps but it didn't help.",
    "I don't have much more to add beyond what I already said.",
    "I'm hoping this can get sorted out fairly quickly.",
    "Let me know if you need anything else from me.",
    "I just want to get this resolved as soon as possible.",
]
...
def make_chat(is_fraud: bool, reason: str):
    ...
    if is_fraud and reason == "Item never arrived" and random.random() < 0.7:
        ...
        lines.append(f"Customer: {random.choice(CUSTOMER_FOLLOWUP_DETAILS)}")
...
def generate():
    """Regenerates the full dataset from the shared seeded random/Faker
    stream. NOTE: re-running this against the committed data/*.json files
    will reshuffle every row (fraud labels, orders, everything), not just
    chat text — make_chat()'s random draws are interleaved with the rest of
    the per-return generation loop, so any change to how many random values
    it consumes shifts the entire downstream sequence. The committed
    data/chats.json currently has its filler lines patched in place instead
    of being regenerated, specifically to avoid that — see git history for
    the patch script if you need to redo it."""
    customers = []
